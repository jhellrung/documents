...
\item[3.] Continuing from the previous problem, show the relation
\begin{equation*}
\lim_{M^-,M^+ \to \infty} \frac{1}{M^+ + M^-} \int_{-M^-}^{M^+} u(t,x) dx = \lim_{M^-,M^+ \to \infty} \frac{1}{M^+ + M^-} \int_{-M^-}^{M^+} u_0(x) dx,
\end{equation*}
