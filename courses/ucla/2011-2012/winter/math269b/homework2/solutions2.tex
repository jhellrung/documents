...
& = \frac{1}{M^2} \sum_{m=0}^{M-1} \left( \sum_{\ell=0}^{M-1} e^{2i\pi\ell m/M} \hat{v}_{\ell} \right) \left( \overline{\sum_{\ell'=0}^{M-1} e^{2i\pi\ell'm/M} \hat{v}_{\ell'}} \right) \\
...
\end{align*}
as desired (since, when $\ell \neq \ell'$, the inner sum over $m$ sums out to $0$).
...
\abs{g}^2 & = \abs{1 - \alpha^2 \left( 1 - \cos \theta \right) + i \alpha \sin \theta}^2 \\
          ...
          & = 1 - \alpha^2 \left( 1 - \cos \theta \right)^2 \left( 1 - \alpha^2 \right),
\end{align*}
