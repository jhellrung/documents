function varargout = rd1dgui(varargin)
% RD1DGUI M-file for rd1dgui.fig
%      RD1DGUI, by itself, creates a new RD1DGUI or raises the existing
%      singleton*.
%
%      H = RD1DGUI returns the handle to a new RD1DGUI or the handle to
%      the existing singleton*.
%
%      RD1DGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in RD1DGUI.M with the given input arguments.
%
%      RD1DGUI('Property','Value',...) creates a new RD1DGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before rd1dgui_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to rd1dgui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help rd1dgui

% Last Modified by GUIDE v2.5 30-Aug-2003 16:59:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @rd1dgui_OpeningFcn, ...
                   'gui_OutputFcn',  @rd1dgui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before rd1dgui is made visible.
function rd1dgui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to rd1dgui (see VARARGIN)

data.buildnew = true;

vars = {'gamma_' 'd' 'xmax' 'J' 'dx' 'dt' 'nu' 'displaystep' ...
...
vars = {'u0t' 'ux0t' 'uxmaxt' 'uxxmaxt' 'u0tuxamxt' ...
        'v0t' 'vx0t' 'vxmaxt' 'vxxmaxt' 'v0tvxmaxt' ...
        'maxperturbation' 'randomseed' 'nsteps' 'maxchange'};
for i = 1:length(vars)
...
% Choose default command line output for rd1dgui
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes rd1dgui wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = rd1dgui_OutputFcn(hObject, eventdata, handles)
    ...
    if isequal(get(handles.forcemaximumprinciplecondition_menu, 'Checked'), 'on')
        ...
        if ~isempty(dx)
            ...
            if ~isempty(dt)
                ...
                J = get(handles.J_textbox, 'Value');
                if ~isempty(xmax)
                    J = max([ceil(xmax/dx) 3]);
                    dx = xmax/J;
                    dt = nu*dx^2;
                elseif ~isempty(J)
                    xmax = J*dx;
                end
                set([handles.xmax_textbox handles.J_textbox handles.dx_textbox], ...
                    {'Value' 'String'}, ...
                    {xmax num2str(xmax,8) ; J int2str(J) ; dx num2str(dx,8)});
            end
        end
...
if ~isempty(string) && isvalid('dt', string)
    ...
    elseif ~isempty(nu)
        ...
        set([handles.xmax_textbox handles.J_textbox], ...
            {'Value' 'String'}, ...
            {xmax num2str(xmax,8) ; J int2str(J)});
    end
    set([handles.dx_textbox handles.dt_textbox handles.nu_textbox], ...
        {'Value' 'String'}, ...
        {dx num2str(dx,8) ; dt num2str(dt,8) ; nu num2str(nu,8)});
else
    set(hObject, 'Value', [], 'String', '');
    if ~isempty(string)
...
%  Boundary Conditions
%    ( ) u(0,t) = 0     ( ) ux(0,t) = 0
%    ( ) u(xmax,t) = 0  ( ) ux(xmax,t) = 0
%    ( ) v(0,t) = 0     ( ) vx(0,t) = 0
%    ( ) v(xmax,t) = 0  ( ) vx(xmax,t) = 0
%
%--------------------------------------------------------------------------
...
if get(handles.u0tuxmaxt_radiobutton, 'Value') == get(handles.u0tuxamxt_radiobutton, 'Max')
    set(handles.uxmaxt_radiobutton, 'Value', get(handles.uxmaxt_radiobutton, 'Max'));
end
set(handles.u0tuxmaxt_radiobutton, 'Value', get(handles.u0tuxmaxt_radiobutton, 'Min'));
...
if get(handles.u0tuxmaxt_radiobutton, 'Value') == get(handles.u0tuxamxt_radiobutton, 'Max')
    set(handles.uxxmaxt_radiobutton, 'Value', get(handles.uxxmaxt_radiobutton, 'Max'));
end
set(handles.u0tuxmaxt_radiobutton, 'Value', get(handles.u0tuxmaxt_radiobutton, 'Min'));



% --- Executes on button press in uxmaxt_radiobutton.
function uxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to uxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of uxmaxt_radiobutton
...
set(handles.uxmaxt_radiobutton, 'Value', get(handles.uxmaxt_radiobutton, 'Max'));
set(handles.uxxmaxt_radiobutton, 'Value', get(handles.uxxmaxt_radiobutton, 'Min'));
if get(handles.u0tuxmaxt_radiobutton, 'Value') == get(handles.u0tuxamxt_radiobutton, 'Max')
    set(handles.u0t_radiobutton, 'Value', get(handles.u0t_radiobutton, 'Max'));
end
set(handles.u0tuxmaxt_radiobutton, 'Value', get(handles.u0tuxmaxt_radiobutton, 'Min'));
...
% --- Executes on button press in uxxmaxt_radiobutton.
function uxxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to uxxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of uxxmaxt_radiobutton

data = getappdata(gcbf, 'data');
data.buildnew = true;
setappdata(gcbf, 'data', data);

set(handles.uxmaxt_radiobutton, 'Value', get(handles.uxmaxt_radiobutton, 'Min'));
set(handles.uxxmaxt_radiobutton, 'Value', get(handles.uxxmaxt_radiobutton, 'Max'));
if get(handles.u0tuxmaxt_radiobutton, 'Value') == get(handles.u0tuxamxt_radiobutton, 'Max')
    set(handles.ux0t_radiobutton, 'Value', get(handles.ux0t_radiobutton, 'Max'));
end
...
% --- Executes on button press in u0tuxmaxt_radiobutton.
function u0tuxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to u0tuxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of u0tuxmaxt_radiobutton

data = getappdata(gcbf, 'data');
data.buildnew = true;
setappdata(gcbf, 'data', data);

set(handles.u0tuxmaxt_radiobutton, 'Value', get(handles.u0tuxmaxt_radiobutton, 'Max'));
set(handles.u0t_radiobutton, 'Value', get(handles.u0t_radiobutton, 'Min'));
set(handles.ux0t_radiobutton, 'Value', get(handles.ux0t_radiobutton, 'Min'));
set(handles.uxmaxt_radiobutton, 'Value', get(handles.uxmaxt_radiobutton, 'Min'));
set(handles.uxxmaxt_radiobutton, 'Value', get(handles.uxxmaxt_radiobutton, 'Min'));
...
if get(handles.v0tvxmaxt_radiobutton, 'Value') == get(handles.v0tvxmaxt_radiobutton, 'Max')
    set(handles.vxmaxt_radiobutton, 'Value', get(handles.vxmaxt_radiobutton, 'Max'));
end
set(handles.v0tvxmaxt_radiobutton, 'Value', get(v0tvxmaxt_radiobutton, 'Min'));
...
if get(handles.v0tvxmaxt_radiobutton, 'Value') == get(handles.v0tvxmaxt_radiobutton, 'Max')
    set(handles.vxxmaxt_radiobutton, 'Value', get(handles.vxxmaxt_radiobutton, 'Max'));
end
set(handles.v0tvxmaxt_radiobutton, 'Value', get(v0tvxmaxt_radiobutton, 'Min'));



% --- Executes on button press in vxmaxt_radiobutton.
function vxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to vxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of vxmaxt_radiobutton
...
set(handles.vxmaxt_radiobutton, 'Value', get(handles.vxmaxt_radiobutton, 'Max'));
set(handles.vxxmaxt_radiobutton, 'Value', get(handles.vxxmaxt_radiobutton, 'Min'));
if get(handles.v0tvxmaxt_radiobutton, 'Value') == get(handles.v0tvxmaxt_radiobutton, 'Max')
    set(handles.v0t_radiobutton, 'Value', get(handles.v0t_radiobutton, 'Max'));
end
set(handles.v0tvxmaxt_radiobutton, 'Value', get(v0tvxmaxt_radiobutton, 'Min'));
...
% --- Executes on button press in vxxmaxt_radiobutton.
function vxxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to vxxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of vxxmaxt_radiobutton

data = getappdata(gcbf, 'data');
data.buildnew = true;
setappdata(gcbf, 'data', data);

set(handles.vxmaxt_radiobutton, 'Value', get(handles.vxmaxt_radiobutton, 'Min'));
set(handles.vxxmaxt_radiobutton, 'Value', get(handles.vxxmaxt_radiobutton, 'Max'));
if get(handles.v0tvxmaxt_radiobutton, 'Value') == get(handles.v0tvxmaxt_radiobutton, 'Max')
    set(handles.vx0t_radiobutton, 'Value', get(handles.vx0t_radiobutton, 'Max'));
end
...
% --- Executes on button press in v0tvxmaxt_radiobutton.
function v0tvxmaxt_radiobutton_Callback(hObject, eventdata, handles)
% hObject    handle to v0tvxmaxt_radiobutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of v0tvxmaxt_radiobutton

data = getappdata(gcbf, 'data');
data.buildnew = true;
setappdata(gcbf, 'data', data);

set(handles.v0tvxmaxt_radiobutton, 'Value', get(handles.v0tvxmaxt_radiobutton, 'Max'));
set(handles.v0t_radiobutton, 'Value', get(handles.v0t_radiobutton, 'Min'));
set(handles.vx0t_radiobutton, 'Value', get(handles.vx0t_radiobutton, 'Min'));
set(handles.vxmaxt_radiobutton, 'Value', get(handles.vxmaxt_radiobutton, 'Min'));
set(handles.vxxmaxt_radiobutton, 'Value', get(handles.vxxmaxt_radiobutton, 'Min'));
...
vars = {'u0t' 'ux0t' 'uxmaxt' 'uxxmaxt' 'u0tuxmaxt' ...
        'v0t' 'vx0t' 'vxmaxt' 'vxxmaxt' 'v0tvxmaxt'};
for i = 1:length(vars)
    s = vars{i};
...
try
    ...
    while step < nsteps && get(handles.hold_pushbutton, 'Value') == 0.0
        ...
        lu(1) = (ux0t_flag*u(2) + u0tuxmaxt_flag*u(J+1)) - 2*u(1) + u(2);
        lu(J+1) = u(J) - 2*u(J+1) + (uxxmaxt_flag*u(J) + u0tuxmaxt_flag*u(1));
        lu = nu*lu;
        if w1(1) ~= 0
            du = du - w1(1)*uu;
        end
        ...
        lv(1) = (vx0t_flag*v(2) + v0tvxmaxt_flag*v(J+1)) - 2*v(1) + v(2);
        lv(J+1) = v(J) - 2*v(J+1) + (vxxmaxt_flag*v(J) + v0tvxmaxt_flag*v(1));
        lv = d*nu*lv;
        if w1(1) ~= 0
            dv = dv - w1(1)*vv;
        end
        ...
        dv(1) = ~v0t_flag*dv(1);
        dv(J+1) = ~vxmaxt_flag*dv(J+1);
        
        
        a = w3(3)*stepsize*nu;
        b = w1(3) + w3(3)*stepsize*2*nu;
        c = w3(3)*stepsize*nu;
        a1 = u0tuxmaxt_flag*a;
        b1 = u0t_flag + ~u0t_flag*b;
        c1 = ~u0t_flag*c + ux0t_flag*a;
        aJ1 = ~uxmaxt_flag*a + uxxmaxt_flag*c;
        bJ1 = uxmaxt_flag + ~uxmaxt_flag*b;
        cJ1 = u0tuxmaxt_flag*c;
        ...
        a = w3(3)*stepsize*d*nu;
        b = w1(3) + w3(3)*stepsize*d*2*nu;
        c = w3(3)*stepsize*d*nu;
        a1 = v0tvxmaxt_flag*a;
        b1 = v0t_flag + ~v0t_flag*b;
        c1 = ~v0t_flag*c + vx0t_flag*a;
        aJ1 = ~vxmaxt_flag*a + vxxmaxt_flag*c;
        bJ1 = vxmaxt_flag + ~vxmaxt_flag*b;
        cJ1 = v0tvxmaxt_flag*c;
...
set(handles.xy_space_menu, 'Checked', 'on');
set(handles.xty_space_menu, 'Checked', 'off');
refreshframe;
...
set(handles.xy_space_menu, 'Checked', 'off');
set(handles.xty_space_menu, 'Checked', 'on');
refreshframe;
...
if length(axislim) == 6
    ...
    if isequal(get(handles.xy_space_menu, 'Checked'), 'on')
        ymin = axislim(3);
        ymax = axislim(4);
    else
        data = getappdata(gcbf, 'data');
...
switch variable
    ...
    case {'positiveinteger' 'displaystep'}
        b = isequal(size(value),[1 1]) && ~isnan(value) && isreal(value) && mod(value,1) == 0 && value > 0;
    case {'positivereal' 'dt' 'dx' 'maxchange' 'maxperturbation' 'nu' 'xmax'}
        b = isequal(size(value),[1 1]) && ~isnan(value) && isreal(value) && value > 0;
        ...
        if isequal(get(eval(['handles.plot' vars{i} '_menu']), 'Checked'), 'on')
            for j = 1:length(colors)
                if isequal(get(eval(['handles.plot' vars{i} '_' colors{j} '_menu']), 'Checked'), 'on')
                    plot(data.x, eval(vars{i}), colors{j});
                end
            end
        end
    end
